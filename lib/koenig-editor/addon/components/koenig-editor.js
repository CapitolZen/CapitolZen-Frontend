...
export const CARD_COMPONENT_MAP = {
  hr: 'koenig-card/hr',
  embedly: 'koenig-card/embedly',
  bill: 'koenig-card/bill'
};
...
export default Component.extend({
  ...
  inputModeDidChange(editor) {
    ...
    let sectionParentTagNames = editor.activeSections.map(
      s => (s.isNested ? s.parent.tagName : s.tagName)
    );
  ...
  handleBackspaceKey() {
    ...
    if (
      ...
      offset === 0 &&
      section.prev &&
      section.prev.type === 'card-section' &&
      !section.isBlank
    ) {
  ...
  _getCardFromSection(section) {
    ...
    let cardId = section.renderNode.element.querySelector('.__mobiledoc-card')
      .firstChild.id;
